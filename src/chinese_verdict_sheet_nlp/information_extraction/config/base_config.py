...
class ConvertArguments:
    labelstudio_file: str = field(
        default="./data/information_extraction/label_studio_data/label_studio_output.json",
        metadata={"help": "The export file from label studio. Only support the JSON format."},
    )

    save_dir: str = field(
        default="./data/information_extraction/model_input_data/",
    ...
    split_ratio: float = field(
        ...
        metadata={
            "help": "The ratio of samples in datasets. [0.7, 0.2, 0.1] means 70% "
            + "samples used for training, 20% for evaluation and 10% for testing."
        },
    )
...
class EvaluationArguments(TrainModelArguments):
    ...
    dev_file: str = field(
        default="./data/information_extraction/model_input_data/test.txt",
...
class InferenceDataArguments:
    data_file: str = field(
        default="./data/information_extraction/model_infer_data/example.txt",
...
class InferenceTaskflowArguments:
    ...
    device_id: int = field(
        default=0,
        metadata={
            "help": "The device id for the gpu. The defalut value is 0. "
            + "If you want to use CPU, set the device_id to -1."
        },
    )
    ...
    precision: str = field(
        default="fp32",
        metadata={
            "help": "fp16 or fp32. Default 'fp32', which is slower than 'fp16'. "
            + "If 'fp16' is applied and gpu is used, make sure your CUDA>=11.2 and cuDNN>=8.1.1."
...
class InferenceStrategyArguments:
    ...
    select_strategy: str = field(
        default="all",
        metadata={
            "help": "'all' or 'max' or 'threshold'. Strategy of getting results. max: Only get the max prob. result. "
            + "all: Get all results. threshold: Ｇet the results whose prob. is larger than threshold."
        },
    )
    ...
    select_key: List[str] = field(
        ...
        metadata={
            "help": "UIE will output ['text', 'start', 'end', 'probability']. "
            + "--select_key is to select which key in the list you want to return."
        },
    )
